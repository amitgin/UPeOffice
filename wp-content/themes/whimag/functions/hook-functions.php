...
?>
<div id="social_box">
<p><a target="_blank" title="<?php _e('Subscribe to our site RSS Feed', 'whimag'); ?>" class="fa fa-rss" href="<?php echo $rss_feed_link; ?>"></a></p>
<?php if($facebook_link) { ?><p><a title="<?php _e('Like us on Facebook', 'whimag'); ?>" class="fa fa-facebook" target="_blank" href="<?php echo $facebook_link; ?>"></a></p><?php } ?>
<?php if($twitter_link) { ?><p><a title="<?php _e('Follow us on Twitter', 'whimag'); ?>" class="fa fa-twitter" target="_blank" href="<?php echo $twitter_link; ?>"></a></p><?php } ?>
<?php if($pinterest_link) { ?><p><a target="_blank" title="<?php _e('Follow us on Pinterest', 'whimag'); ?>" class="fa fa-pinterest" href="<?php echo $pinterest_link; ?>"></a></p><?php } ?>
<?php if($google_plus_link) { ?><p><a target="_blank" title="<?php _e('Join us on Google Page', 'whimag'); ?>" class="fa fa-google" href="<?php echo $google_plus_link; ?>"></a></p><?php } ?>
...
<nav class="primary-nav" id="main-navigation"<?php do_action('mp_section_main_nav'); ?>>
<div class="innerwrap">
...
frkw_init_mobile_menu('mobile', __('Browse The Site','whimag') );
} }
add_action('mp_inside_header','frkw_add_mobile_menu', 80);
...
add_action('mp_inside_header','frkw_add_header_ads');
...
add_action('mp_inside_wrapper','frkw_custom_img_header',10);
...
<h3 class="widget-title"><?php _e('About This Theme', 'whimag'); ?></h3>
...
<?php frkw_get_popular_posts('disable','enable',5); ?>
</aside>
<?php endif; ?>
</div>
</div>
...
<?php else: ?>
<aside class="widget">
<h3 class="widget-title"><?php _e('Tags','whimag'); ?></h3>
...
<?php }
add_action('mp_before_end_wrapper','frkw_add_footer_bottom_html');

/*--------------------------------------------
Description: Add footer nav
---------------------------------------------*/
function frkw_add_footer_copyright() {
echo __('Copyright &copy;', 'whimag') . gmdate('Y') . ' ' . get_bloginfo('name');
}
...
echo '<ul><li>' . frkw_author_info() . '</li></ul>';
}
}
...
echo frkw_get_featured_image("<div class='post-thumb'>".$thepostlink, "</a></div>", 300, 300, "alignnone", 'thumbnail', frkw_get_image_alt_text(),the_title_attribute('echo=0'), false);
}
}
add_action('mp_before_the_content','frkw_add_archive_thumbnail');
...
---------------------------------------------*/
function frkw_add_entry_category_meta() { if( !is_singular() ) { ?>
<span class="entry-category home-entry-category"><?php echo frkw_get_singular_cat(); ?></span>
<?php } }
//add_action('mp_before_post_title','frkw_add_entry_category_meta');
...
$form = '<form method="get" class="search-form" action="' . esc_url( home_url( '/' ) ) . '"><label><span class="screen-reader-text">' . __( 'Search for:', 'whimag' ) . '</span><input type="search" class="search-field" placeholder="' . __( 'Search &hellip;', 'whimag' ) . '" value="' . get_search_query() . '" name="s" title="' . __( 'Search for:', 'whimag' ) . '" /></label> <input type="submit" class="search-submit" value="'. __( 'Search', 'whimag' ) .'" /></form>';
return $form;
}
...
$home = __('Home', 'whimag'); // text for the 'Home' link
...
elseif ( is_search()) {
echo __('Search results for', 'whimag') . ' "' . get_search_query() . '"';
}
elseif ( is_day()) {
...
echo ' ' . $delimiter . ' ' . __('You are reading &raquo;', 'whimag');
}
else {
...
echo __('You are reading &raquo;', 'whimag');
}
}
...
elseif ( is_404()) {
echo ' ' . $delimiter . ' ' . __('Error 404', 'whimag');
}
if ( get_query_var('paged')) {
...
---------------------------------------------*/
function frkw_add_itemtype_article() { echo ' itemscope="" itemtype="http://schema.org/Article"'; }
function frkw_add_itemtype_post_title() { echo ' itemprop="name headline"'; }
function frkw_add_itemtype_post_content() { echo ' itemprop="articleBody"'; }
...
if( $schema_on == 'enable' ) {
add_filter('the_content', 'frkw_add_custom_schema');
