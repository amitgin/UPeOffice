...
<?php echo frkw_get_featured_image($thepostlink, "</a>", 250, 250, "aligncenter", "thumbnail",frkw_get_image_alt_text(),the_title_attribute('echo=0'), true); ?>
</div>
...
if ($categories) {
	...
	if( $my_query->have_posts() ) {
    echo '<div id="related-posts">' . '<h4>';
     echo _e( 'Related Articles', 'whimag' );
    echo '</h4>';
		while ($my_query->have_posts()) {
		$my_query->the_post();
        $thepostlink = '<a href="'. get_permalink() . '" title="' . the_title_attribute('echo=0') . '">';
