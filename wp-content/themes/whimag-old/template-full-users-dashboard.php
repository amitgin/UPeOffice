...
<style>
    ...
    #xyz {width:100%;height:100%;top:0;left:0;display:none;position:fixed;background-color:#313131;overflow:auto}
                ...
                <table>
                    <tbody>
                       <tr>
                        ...
                        <?php
                            $args = array('role' => 'contributor', 'orderby' => 'display_name');
                            $users = get_users($args);
                            foreach($users as $page){
                                $contributorNo = $wpdb->get_var("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login'");
                                $myquery = $wpdb->get_results("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login' GROUP BY department_name");
                                $numofdepartment = $wpdb->num_rows;
                                $numofdsc = $wpdb->get_var("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login' AND digsign = 'Yes'");
                                $numofcomputer = $wpdb->get_var("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login' AND computer = 'Computer,'");
                                $numofscanner = $wpdb->get_var("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login' AND scanner = 'Scanner,'");
                                $numofinternet = $wpdb->get_var("SELECT COUNT(*) FROM uptron_contributor_list where userid = '$page->user_login' AND internet = 'Internet'");
                                echo '<tr>
                                    <td style="text-align:center">'.$i++.'</td>
                                    <td>'.$page->display_name.'&nbsp;('.$page->user_login.')</td>
        ...
        <div class="" id="">
                        ...
                        <?php global $wpdb;
                            ...
                            foreach($news as $page){
                                echo '<tr>
                                        ...
                                        <td><a href="/govt-orders/'.$page->attachment.'">'.$page->attachment.'</a></td>
                                        <td style="text-align:center;">'.$page->postdate.'</td>
                                        <td style="text-align:center;"><button value="Edit" onclick=editEvent('.$page->id.')>Edit</button></td>
                                    </tr>';
                            }
                        ?>
                    </tbody>
            ...
            <script>
                ...
                function editEvent(id){
                    $.get("/report/editReport.php?id="+id, function(data, status){
                        ...
                        if(in_array($attachmentActualExt, $allowed)){
                            if($attachment_error===0){
                                if($attachment_size < 1000000){
                                    ...
                                    $attachmentDestination = 'govt-orders/'.$attachment_name;
...
?>
        ...
        <?php
             function runMyFunction() {
                 ...
                 $department = $wpdb->get_results("SELECT `department_name` FROM `uptron_contributor_list` WHERE `userid`='$contributerid' GROUP BY `department_name`");
        ?>
...
<script>
	...
	if (document.getElementById('name').value == "" || document.getElementById('department_name').value == "" || document.getElementById('organization').value == "" || document.getElementById('post_name').value == "" || document.getElementById('aadhaar_number').value == "" || document.getElementById('mobile_number').value == "" || document.getElementById('cug_number').value == "" || document.getElementById('email_id').value == "" || || document.getElementById('digsign').value == "") {
